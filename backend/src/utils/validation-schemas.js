  ...
  auth: {
    register: {
      ...
      password: {
        ...
        required: function (data) {
          ...
          // If idToken or provider is present, it's an OAuth registration
          return !data.idToken && !data.provider;
        },
      ...
      // Add fields for OAuth registration
      idToken: {
        type: "string",
        required: false,
      },
    ...
    updateRole: {
      role: {
        ...
        message:
          "Invalid role. Must be one of: user, admin, moderator, counselor",
      },
    },
  ...
  events: {
    create: {
      ...
      category: {
        ...
        message:
          "Invalid category. Must be one of: workshop, seminar, conference, meetup, other",
      },
      capacity: {
        type: "number",
        required: false,
...
export const validateData = (data, schema) => {
  ...
  for (const [field, rules] of Object.entries(schema)) {
    ...
    if (typeof rules.required === "function") {
      isRequired = rules.required(data);
    }
    ...
    if (
      ...
      errors.push({
        field,
        message:
          rules.message || VALIDATION_MESSAGES.min(field, rules.minLength),
      });
    }
    ...
    if (
      ...
      errors.push({
        field,
        message:
          rules.message || VALIDATION_MESSAGES.max(field, rules.maxLength),
      });
    }
